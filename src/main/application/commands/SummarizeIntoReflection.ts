...
export class SummarizeIntoReflection {
  ...
  async execute(matchId: string, sessionId: string, messages: ChatTurn[]): Promise<CoachChatReply> {
    ...
    const renderRefs = makeRefLineRenderer(match.report, match.standings)
    ...
    const validRefIds = new Set<string>(buildAnchorCatalog(match.report).keys())
    for (const t of match.standings ?? []) validRefIds.add(`task:${t.id}`)
