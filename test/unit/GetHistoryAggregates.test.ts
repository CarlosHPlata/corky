...
describe('GetHistoryAggregates', () => {
  it('builds rows from stored raw JSON and aggregates the cohort', () => {
    const repo = fakeRepo([
    ...
    const agg = new GetHistoryAggregates(repo).execute(TARGET)
  ...
  it('excludes the analysed match from its own cohort', () => {
    const repo = fakeRepo([
    ...
    const agg = new GetHistoryAggregates(repo).execute({ ...TARGET, excludeMatchId: 'SELF' })
    expect(agg!.games).toBe(1)
    expect(agg!.averages.deaths).toBe(3)
  })

  it('returns null when no account is synced or nothing is stored', () => {
    expect(new GetHistoryAggregates(fakeRepo([], false)).execute(TARGET)).toBeNull()
    expect(new GetHistoryAggregates(fakeRepo([])).execute(TARGET)).toBeNull()
  })

  it('skips unparseable stored matches instead of failing', () => {
    const repo = fakeRepo([
